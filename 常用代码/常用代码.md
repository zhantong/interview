...
- [41. 十进制转换为7进制](#41-十进制转换为7进制)
- [42. 指定长度二进制数，指定1的个数的所有可能情况](#42-指定长度二进制数指定1的个数的所有可能情况)
- [43. 有序数组转二叉搜索树](#43-有序数组转二叉搜索树)
- [44. 最大子数列问题](#44-最大子数列问题)
  - [44.1. 最小值为0](#441-最小值为0)
  - [44.2. 最小值为负数](#442-最小值为负数)
- [45. 判断一个数是否为2的幂次](#45-判断一个数是否为2的幂次)
- [46. 判断一个数是否为4的幂次](#46-判断一个数是否为4的幂次)
- [47. 最近公共祖先问题](#47-最近公共祖先问题)
  - [47.1. 二叉树的最近公共祖先](#471-二叉树的最近公共祖先)
  - [47.2. 二叉搜索树的最近公共祖先](#472-二叉搜索树的最近公共祖先)
- [48. 挑选数组中不连续的元素，使其和最大](#48-挑选数组中不连续的元素使其和最大)
- [49. 镜像二叉树](#49-镜像二叉树)
- [50. 判断二叉树是否平衡](#50-判断二叉树是否平衡)
- [51. 二分法求平方根](#51-二分法求平方根)
- [52. n!求末尾0的个数](#52-n求末尾0的个数)
- [53. 有序数组去重](#53-有序数组去重)
- [54. 链表判断环](#54-链表判断环)
- [55. 判断是否为回文数](#55-判断是否为回文数)
- [56. 是否存在二叉树根到叶子节点路径和为指定值](#56-是否存在二叉树根到叶子节点路径和为指定值)
- [57. 找出字符串中所有变位词](#57-找出字符串中所有变位词)
- [58. 判断两个字符串是否为同构字符串](#58-判断两个字符串是否为同构字符串)
- [59. 判断一个数是否为其全部约数的和](#59-判断一个数是否为其全部约数的和)
- [60. 打乱数组元素](#60-打乱数组元素)
- [61. 生成不重复随机序列](#61-生成不重复随机序列)
- [62. 使用随机生成0到5的函数，实现随机生成0到7的函数](#62-使用随机生成0到5的函数实现随机生成0到7的函数)
- [63. 计算数组A、B、C、D中有多少种(i, j, k, l)使得A\[i\] + B\[j\] + C\[k\] + D\[l\] = 0](#63-计算数组abcd中有多少种i-j-k-l使得ai--bj--ck--dl--0)
- [64. 计算给出的多个时间中，最小的时间差](#64-计算给出的多个时间中最小的时间差)
- [65. 计算\[0, 10^n)中各位数字不同的数的个数](#65-计算\0-10^n中各位数字不同的数的个数)
- [66. 数组中仅有两个数仅出现一次，其余的数均出现两次，找出这两个数](#66-数组中仅有两个数仅出现一次其余的数均出现两次找出这两个数)
- [67. 找出递增数列中第n位的数字](#67-找出递增数列中第n位的数字)
- [68. 计算小于n的质数的个数](#68-计算小于n的质数的个数)
- [69. 由部分（可重复）给定数组元素相加得到指定值的情况数](#69-由部分可重复给定数组元素相加得到指定值的情况数)
- [70. 在有序矩阵中寻找第k大的数](#70-在有序矩阵中寻找第k大的数)
- [71. 求n对括号的所有排列情况](#71-求n对括号的所有排列情况)
- [72. 给定字符串数组，求最大不相交的两个字符串的长度积](#72-给定字符串数组求最大不相交的两个字符串的长度积)
- [73. 在由\[1, n\]组成的长为n + 1的数组中，寻找重复的那个数](#73-在由1-n组成的长为n--1的数组中寻找重复的那个数)
- [74. 求给定数组的所有排列情况](#74-求给定数组的所有排列情况)
- [75. 最长回文子序列](#75-最长回文子序列)
- [76. 将\[1, n\]的数按字典排序](#76-将1-n的数按字典排序)
- [77. 二叉搜索树迭代器](#77-二叉搜索树迭代器)
- [78. 由\[1, n\]组成的二叉搜索树的个数](#78-由1-n组成的二叉搜索树的个数)
- [79. 格雷码](#79-格雷码)
- [80. 从m x n棋盘左上角走到右下角的路径总数](#80-从m-x-n棋盘左上角走到右下角的路径总数)
- [81. 求循环移位的有序数组中的到最小值](#81-求循环移位的有序数组中的到最小值)
- [82. 求集合的所有子集](#82-求集合的所有子集)
- [83. 判断数组arr中是否存在arr\[i\] &lt; arr\[j\] &lt; arr\[k\]](#83-判断数组arr中是否存在arri-lt-arrj-lt-arrk)
- [84. 查找数组中第k大的数](#84-查找数组中第k大的数)
- [85. 链表删除倒数第n个结点](#85-链表删除倒数第n个结点)
- [86. 判断数组是否可以拼接为正方形](#86-判断数组是否可以拼接为正方形)
- [87. 快速`pow()`](#87-快速pow)
- [88. 判断数组是否可以分为相等的两部分](#88-判断数组是否可以分为相等的两部分)
- [89. 在矩阵中查找指定值](#89-在矩阵中查找指定值)
- [90. 将矩阵顺时针旋转90度](#90-将矩阵顺时针旋转90度)
- [91. 最长递增子序列](#91-最长递增子序列)
- [92. 求二维矩阵由左上角到右下角的最小和](#92-求二维矩阵由左上角到右下角的最小和)
- [93. 将由0、1、2组成的数组排序](#93-将由012组成的数组排序)
- [94. 求第n位仅由指定因数组成的数字](#94-求第n位仅由指定因数组成的数字)
- [95. 数组中查找极大值](#95-数组中查找极大值)
- [96. 判断n能否由平方数的和组成](#96-判断n能否由平方数的和组成)
- [97. 删除二叉搜索树中的结点](#97-删除二叉搜索树中的结点)
- [98. 检查字符串是否是合法的二叉树前序遍历结果](#98-检查字符串是否是合法的二叉树前序遍历结果)
- [99. 中缀表达式转后缀表达式](#99-中缀表达式转后缀表达式)
- [100. 快速替换10亿条标题中的5万个敏感词](#100-快速替换10亿条标题中的5万个敏感词)
- [101. 轮流取数的必胜策略](#101-轮流取数的必胜策略)
- [102. 最大公约数（辗转相除法）](#102-最大公约数辗转相除法)
- [103. 最小公倍数](#103-最小公倍数)
...
## 63. 计算数组A、B、C、D中有多少种(i, j, k, l)使得A[i] + B[j] + C[k] + D[l] = 0
...
## 66. 数组中仅有两个数仅出现一次，其余的数均出现两次，找出这两个数
...
## 73. 在由[1, n]组成的长为n + 1的数组中，寻找重复的那个数
...
## 83. 判断数组arr中是否存在arr[i] &lt; arr[j] &lt; arr[k]
