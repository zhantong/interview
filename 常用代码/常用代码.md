...
- [12. 链表排序](#12-链表排序)
- [13. 检查回文链表](#13-检查回文链表)
- [14. 链表等概率随机抽取元素](#14-链表等概率随机抽取元素)
- [15. 链表按奇数序号和偶数序号重排](#15-链表按奇数序号和偶数序号重排)
- [16. 二叉树前序遍历](#16-二叉树前序遍历)
  - [16.1. 递归](#161-递归)
  - [16.2. 非递归](#162-非递归)
- [17. 二叉树中序遍历](#17-二叉树中序遍历)
  - [17.1. 递归](#171-递归)
  - [17.2. 非递归](#172-非递归)
- [18. 二叉树后序遍历](#18-二叉树后序遍历)
  - [18.1. 递归](#181-递归)
  - [18.2. 非递归](#182-非递归)
- [19. 二叉树层次遍历](#19-二叉树层次遍历)
- [20. Dijkstra算法](#20-dijkstra算法)
  - [20.1. 朴素](#201-朴素)
  - [20.2. 堆优化](#202-堆优化)
- [21. Floyd-Warshall算法](#21-floyd-warshall算法)
- [22. 拓扑排序](#22-拓扑排序)
  - [22.1. DFS](#221-dfs)
  - [22.2. BFS](#222-bfs)
- [23. 二分搜索](#23-二分搜索)
...
- [27. 栈模拟队列](#27-栈模拟队列)
- [28. 队列模拟栈](#28-队列模拟栈)
- [29. 生产者——消费者模型](#29-生产者消费者模型)
- [30. LRU](#30-lru)
- [31. LFU](#31-lfu)
- [32. 自顶向下](#32-自顶向下)
- [33. 自底向上](#33-自底向上)
- [34. 计算数字二进制表示的1的个数](#34-计算数字二进制表示的1的个数)
- [35. 数组中除一个数仅出现一次外，其他数均出现两次，找出这个数](#35-数组中除一个数仅出现一次外其他数均出现两次找出这个数)
- [36. 长为n的数组中所有元素在\[1, n\]之间，元素最多出现两次，找出\[1, n\]中没出现的元素](#36-长为n的数组中所有元素在1-n之间元素最多出现两次找出1-n中没出现的元素)
- [37. 不用`+`和`-`计算两个数的和](#37-不用和-计算两个数的和)
- [38. 反转二叉树](#38-反转二叉树)
- [39. 把数组中所有0移动到尾部，数组元素保持相对顺序不变](#39-把数组中所有0移动到尾部数组元素保持相对顺序不变)
- [40. 定义一次move是将长为n的数组中n-1个数均加1，最少多少个move可以使得数组中所有元素相等](#40-定义一次move是将长为n的数组中n-1个数均加1最少多少个move可以使得数组中所有元素相等)
- [41. 找出长为n的数组中出现次数大于⌊ n/2 ⌋的元素](#41-找出长为n的数组中出现次数大于⌊-n2-⌋的元素)
- [42. 找出长为n的数组中出现次数大于⌊ n/k ⌋的元素](#42-找出长为n的数组中出现次数大于⌊-nk-⌋的元素)
- [43. 十进制转换为7进制](#43-十进制转换为7进制)
- [44. 指定长度二进制数，指定1的个数的所有可能情况](#44-指定长度二进制数指定1的个数的所有可能情况)
- [45. 有序数组转二叉搜索树](#45-有序数组转二叉搜索树)
- [46. 最大连续子序列和](#46-最大连续子序列和)
  - [46.1. 最小值为0](#461-最小值为0)
  - [46.2. 最小值为负数](#462-最小值为负数)
- [47. 最大连续子序列积](#47-最大连续子序列积)
- [48. 判断一个数是否为2的幂次](#48-判断一个数是否为2的幂次)
- [49. 判断一个数是否为4的幂次](#49-判断一个数是否为4的幂次)
- [50. 最近公共祖先问题](#50-最近公共祖先问题)
  - [50.1. 二叉树的最近公共祖先](#501-二叉树的最近公共祖先)
  - [50.2. 二叉搜索树的最近公共祖先](#502-二叉搜索树的最近公共祖先)
- [51. 挑选数组中不连续的元素，使其和最大](#51-挑选数组中不连续的元素使其和最大)
- [52. 镜像二叉树](#52-镜像二叉树)
- [53. 判断二叉树是否平衡](#53-判断二叉树是否平衡)
- [54. 二分法求平方根](#54-二分法求平方根)
- [55. n!求末尾0的个数](#55-n求末尾0的个数)
- [56. 有序数组去重](#56-有序数组去重)
- [57. 链表判断环](#57-链表判断环)
- [58. 有环链表获取环起始结点](#58-有环链表获取环起始结点)
- [59. 判断是否为回文数](#59-判断是否为回文数)
- [60. 是否存在二叉树根到叶子节点路径和为指定值](#60-是否存在二叉树根到叶子节点路径和为指定值)
- [61. 找出字符串中所有变位词](#61-找出字符串中所有变位词)
- [62. 判断两个字符串是否为同构字符串](#62-判断两个字符串是否为同构字符串)
- [63. 判断一个数是否为其全部约数的和](#63-判断一个数是否为其全部约数的和)
- [64. 打乱数组元素](#64-打乱数组元素)
- [65. 生成不重复随机序列](#65-生成不重复随机序列)
- [66. 使用随机生成0到5的函数，实现随机生成0到7的函数](#66-使用随机生成0到5的函数实现随机生成0到7的函数)
- [67. 计算数组A、B、C、D中有多少种(i, j, k, l)使得A\[i\] + B\[j\] + C\[k\] + D\[l\] = 0](#67-计算数组abcd中有多少种i-j-k-l使得ai--bj--ck--dl--0)
- [68. 计算给出的多个时间中，最小的时间差](#68-计算给出的多个时间中最小的时间差)
- [69. 计算\[0, 10^n)中各位数字不同的数的个数](#69-计算\0-10^n中各位数字不同的数的个数)
- [70. 数组中仅有两个数仅出现一次，其余的数均出现两次，找出这两个数](#70-数组中仅有两个数仅出现一次其余的数均出现两次找出这两个数)
- [71. 找出递增数列中第n位的数字](#71-找出递增数列中第n位的数字)
- [72. 计算小于n的质数的个数](#72-计算小于n的质数的个数)
- [73. 由部分（可重复）给定数组元素相加得到指定值的情况数](#73-由部分可重复给定数组元素相加得到指定值的情况数)
- [74. 在有序矩阵中寻找第k大的数](#74-在有序矩阵中寻找第k大的数)
- [75. 求n对括号的所有排列情况](#75-求n对括号的所有排列情况)
- [76. 给定字符串数组，求最大不相交的两个字符串的长度积](#76-给定字符串数组求最大不相交的两个字符串的长度积)
- [77. 在由\[1, n\]组成的长为n + 1的数组中，寻找重复的那个数](#77-在由1-n组成的长为n--1的数组中寻找重复的那个数)
- [78. 求给定数组的所有排列情况](#78-求给定数组的所有排列情况)
- [79. 求给定（元素可重复）数组的所有排列情况](#79-求给定元素可重复数组的所有排列情况)
- [80. 最长回文子序列](#80-最长回文子序列)
- [81. 将\[1, n\]的数按字典排序](#81-将1-n的数按字典排序)
- [82. 二叉搜索树迭代器](#82-二叉搜索树迭代器)
- [83. 由\[1, n\]组成的二叉搜索树的个数](#83-由1-n组成的二叉搜索树的个数)
- [84. 格雷码](#84-格雷码)
- [85. 从m x n棋盘左上角走到右下角的路径总数](#85-从m-x-n棋盘左上角走到右下角的路径总数)
- [86. 求循环移位的有序数组中的到最小值](#86-求循环移位的有序数组中的到最小值)
- [87. 求集合的所有子集](#87-求集合的所有子集)
- [88. 求（元素可重复）集合的所有子集](#88-求元素可重复集合的所有子集)
- [89. 判断数组arr中是否存在arr\[i\] &lt; arr\[j\] &lt; arr\[k\]](#89-判断数组arr中是否存在arri-lt-arrj-lt-arrk)
- [90. 查找数组中第k大的数](#90-查找数组中第k大的数)
- [91. 链表删除倒数第n个结点](#91-链表删除倒数第n个结点)
- [92. 判断数组是否可以拼接为正方形](#92-判断数组是否可以拼接为正方形)
- [93. 快速`pow()`](#93-快速pow)
- [94. 判断数组是否可以分为相等的两部分](#94-判断数组是否可以分为相等的两部分)
- [95. 在矩阵中查找指定值](#95-在矩阵中查找指定值)
- [96. 将矩阵顺时针旋转90度](#96-将矩阵顺时针旋转90度)
- [97. 最长递增子序列](#97-最长递增子序列)
- [98. 求二维矩阵由左上角到右下角的最小和](#98-求二维矩阵由左上角到右下角的最小和)
- [99. 将由0、1、2组成的数组排序](#99-将由012组成的数组排序)
- [100. 求第n位仅由指定因数组成的数字](#100-求第n位仅由指定因数组成的数字)
- [101. 数组中查找极大值](#101-数组中查找极大值)
- [102. 判断n能否由平方数的和组成](#102-判断n能否由平方数的和组成)
- [103. 删除二叉搜索树中的结点](#103-删除二叉搜索树中的结点)
- [104. 检查字符串是否是合法的二叉树前序遍历结果](#104-检查字符串是否是合法的二叉树前序遍历结果)
- [105. 中缀表达式转后缀表达式](#105-中缀表达式转后缀表达式)
- [106. 排列的下一种情况](#106-排列的下一种情况)
- [107. 求旋转有序数组的最小值](#107-求旋转有序数组的最小值)
- [108. 在旋转有序数组中查找指定值](#108-在旋转有序数组中查找指定值)
- [109. 在旋转（元素可重复）有序数组中查找指定值](#109-在旋转元素可重复有序数组中查找指定值)
- [110. 有序数组中查找特定元素的第一个和最后一个索引](#110-有序数组中查找特定元素的第一个和最后一个索引)
- [111. 有序数组中查找指定值的插入位置](#111-有序数组中查找指定值的插入位置)
- [112. 实现`pow(x, n)`](#112-实现powx-n)
- [113. 第k个排列](#113-第k个排列)
- [114. 链表向右循环移动k个位置](#114-链表向右循环移动k个位置)
- [115. 先序遍历和中序遍历构建二叉树](#115-先序遍历和中序遍历构建二叉树)
- [116. 中序遍历和后序遍历构建二叉树](#116-中序遍历和后序遍历构建二叉树)
- [117. 求杨辉三角第k行的值](#117-求杨辉三角第k行的值)
- [118. 克隆图](#118-克隆图)
- [119. 查找数组中的极大值元素](#119-查找数组中的极大值元素)
- [120. 将数组合并拼接为最大的数](#120-将数组合并拼接为最大的数)
- [121. 计算完全二叉树的结点数](#121-计算完全二叉树的结点数)
- [122. 加减乘除（无括号）计算器](#122-加减乘除无括号计算器)
- [123. 加减（带括号）计算器](#123-加减带括号计算器)
- [124. 二叉搜索树中第k小的数](#124-二叉搜索树中第k小的数)
- [125. 求（元素可更新）数组指定区间的和](#125-求元素可更新数组指定区间的和)
- [126. 求数组中任意两个元素异或的最大值](#126-求数组中任意两个元素异或的最大值)
- [127. 二叉搜索树的序列化和反序列化](#127-二叉搜索树的序列化和反序列化)
- [128. 轮流取数判断先取者是否必胜](#128-轮流取数判断先取者是否必胜)
- [129. 一串数加上分别加上加号或减号使其结果等于指定值](#129-一串数加上分别加上加号或减号使其结果等于指定值)
- [130. 有序数组中除一个数仅出现一次外，其他数均出现两次，求这个数](#130-有序数组中除一个数仅出现一次外其他数均出现两次求这个数)
- [131. 最长任意元素差最大为1的子序列](#131-最长任意元素差最大为1的子序列)
- [132. 数组中寻找任意三个元素的最大积](#132-数组中寻找任意三个元素的最大积)
- [133. 快速替换10亿条标题中的5万个敏感词](#133-快速替换10亿条标题中的5万个敏感词)
- [134. 轮流取数的必胜策略](#134-轮流取数的必胜策略)
- [135. 最大公约数（辗转相除法）](#135-最大公约数辗转相除法)
- [136. 最小公倍数](#136-最小公倍数)
...
## 35. 数组中除一个数仅出现一次外，其他数均出现两次，找出这个数
...
## 36. 长为n的数组中所有元素在[1, n]之间，元素最多出现两次，找出[1, n]中没出现的元素
...
## 40. 定义一次move是将长为n的数组中n-1个数均加1，最少多少个move可以使得数组中所有元素相等
...
## 67. 计算数组A、B、C、D中有多少种(i, j, k, l)使得A[i] + B[j] + C[k] + D[l] = 0
...
## 70. 数组中仅有两个数仅出现一次，其余的数均出现两次，找出这两个数
...
## 77. 在由[1, n]组成的长为n + 1的数组中，寻找重复的那个数
...
## 89. 判断数组arr中是否存在arr[i] &lt; arr[j] &lt; arr[k]
...
## 130. 有序数组中除一个数仅出现一次外，其他数均出现两次，求这个数
