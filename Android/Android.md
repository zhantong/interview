...
- [7. Activity的启动模式？](#7-activity的启动模式)
- [8. 横竖屏切换时候activity的生命周期？](#8-横竖屏切换时候activity的生命周期)
- [9. 如何将一个Activity设置成窗口的样式？](#9-如何将一个activity设置成窗口的样式)
- [10. Activity之间的数据传递有哪些方式？](#10-activity之间的数据传递有哪些方式)
- [11. Fragment的好处：](#11-fragment的好处)
- [12. Intent的原理，作用，可以传递哪些类型的参数？](#12-intent的原理作用可以传递哪些类型的参数)
- [13. Service的启动方式](#13-service的启动方式)
- [14. Service的生命周期](#14-service的生命周期)
- [15. Activity怎么和Service绑定，怎么在Activity中启动自己对应的Service？](#15-activity怎么和service绑定怎么在activity中启动自己对应的service)
- [16. 允许绑定的已启动服务的生命周期](#16-允许绑定的已启动服务的生命周期)
- [17. Service中可以弹Toast吗？](#17-service中可以弹toast吗)
- [18. 进程的优先级](#18-进程的优先级)
    - [18.1. 前台进程](#181-前台进程)
    - [18.2. 可视进程](#182-可视进程)
    - [18.3. 服务进程](#183-服务进程)
    - [18.4. 后台进程](#184-后台进程)
    - [18.5. 空进程](#185-空进程)
- [19. IntentService与Service的区别？](#19-intentservice与service的区别)
- [20. Android Service与Activity之间的通信方式？](#20-android-service与activity之间的通信方式)
- [21. BroadcastReceiver简介](#21-broadcastreceiver简介)
    - [21.1. 用途](#211-用途)
    - [21.2. 使用场景](#212-使用场景)
    - [21.3. 实现原理](#213-实现原理)
    - [21.4. 注册方式](#214-注册方式)
- [22. 为什么要用ContentProvider？它和SQL的实现上有什么差别？](#22-为什么要用contentprovider它和sql的实现上有什么差别)
- [23. ContentProvider怎么实现数据共享？](#23-contentprovider怎么实现数据共享)
- [24. Android如何访问自定义ContentProvider](#24-android如何访问自定义contentprovider)
- [25. Android中Activity，Intent，Content Provider，Service各有什么区别。](#25-android中activityintentcontent-providerservice各有什么区别)
- [26. Android数据存储方式？](#26-android数据存储方式)
- [27. Android中常用的布局都有哪些？](#27-android中常用的布局都有哪些)
- [28. `android:layout_gravity`和`android:gravity`的区别？](#28-androidlayout_gravity和androidgravity的区别)
- [29. Android平台架构](#29-android平台架构)
    - [29.1. 系统应用](#291-系统应用)
    - [29.2. Java API框架](#292-java-api框架)
    - [29.3. 原生C/C++库](#293-原生cc库)
    - [29.4. Android Runtime](#294-android-runtime)
    - [29.5. 硬件抽象层（HAL）](#295-硬件抽象层hal)
    - [29.6. Linux 内核](#296-linux-内核)
- [30. Fragment生命周期](#30-fragment生命周期)
- [31. Activity生命周期对片段生命周期的影响](#31-activity生命周期对片段生命周期的影响)
- [32. Android事件分发](#32-android事件分发)
    - [32.1. public boolean dispatchTouchEvent(MotionEvent ev)](#321-public-boolean-dispatchtoucheventmotionevent-ev)
    - [32.2. `public boolean onInterceptTouchEvent(MotionEvent ev)`](#322-public-boolean-onintercepttoucheventmotionevent-ev)
    - [32.3. `public boolean onTouchEvent(MotionEvent ev)`](#323-public-boolean-ontoucheventmotionevent-ev)
- [33. Android系统启动过程](#33-android系统启动过程)
    - [33.1. Boot ROM](#331-boot-rom)
    - [33.2. Boot Loader](#332-boot-loader)
    - [33.3. Kernel](#333-kernel)
    - [33.4. init](#334-init)
        - [33.4.1. 本地服务](#3341-本地服务)
        - [33.4.2. Android服务](#3342-android服务)
    - [33.5. Zygote and Dalvik（ART）](#335-zygote-and-dalvikart)
    - [33.6. System Server](#336-system-server)
    - [33.7. Boot completed](#337-boot-completed)
- [34. Android应用启动过程](#34-android应用启动过程)
- [35. dp，dip，dpi，ppi区别](#35-dpdipdpippi区别)
- [36. 长度和字体的推荐单位](#36-长度和字体的推荐单位)
- [37. Android View绘制流程](#37-android-view绘制流程)
- [38. ListView优化](#38-listview优化)
    - [38.1. 复用convertView](#381-复用convertview)
    - [38.2. 使用View Holder模式](#382-使用view-holder模式)
    - [38.3. 分批加载与分页加载相结合](#383-分批加载与分页加载相结合)
    - [38.4. 使用异步线程加载图片](#384-使用异步线程加载图片)
    - [38.5. 在快速滑动时不要加载图片](#385-在快速滑动时不要加载图片)
    - [38.6. 使用RecyclerView](#386-使用recyclerview)
- [39. Android Binder机制](#39-android-binder机制)
- [40. Binder机制优点](#40-binder机制优点)
- [41. AsyncTask简介](#41-asynctask简介)
- [42. 为什了Handler需要声明为static？](#42-为什了handler需要声明为static)
- [43. 广播注册后不解除注册会有什么问题？](#43-广播注册后不解除注册会有什么问题)
- [44. 自定义View](#44-自定义view)
    - [44.1. 实现步骤](#441-实现步骤)
- [45. 需要被重写的方法](#45-需要被重写的方法)
- [46. Parcelable和Serializable的区别](#46-parcelable和serializable的区别)
- [47. Android中的内存泄漏](#47-android中的内存泄漏)
- [48. MVC和MVP的区别](#48-mvc和mvp的区别)
- [49. 内存泄露检测有什么好方法？](#49-内存泄露检测有什么好方法)
- [50. Android里面为什么要设计出Bundle而不是直接用Map结构](#50-android里面为什么要设计出bundle而不是直接用map结构)
- [51. 在Android的MVP架构中，使用了什么设计模式](#51-在android的mvp架构中使用了什么设计模式)
- [52. Android动画类型](#52-android动画类型)
- [53. ANR和FC的区别](#53-anr和fc的区别)
- [54. Android中的菜单](#54-android中的菜单)
    - [54.1. 选项菜单（Options menu）](#541-选项菜单options-menu)
    - [54.2. 上下文菜单（Contextual Menus）](#542-上下文菜单contextual-menus)
        - [54.2.1. 浮动上下文菜单（floating context menu）](#5421-浮动上下文菜单floating-context-menu)
    - [54.3. 弹出菜单（Popup Menu）](#543-弹出菜单popup-menu)
- [55. BaseAdapter中需要重载的方法](#55-baseadapter中需要重载的方法)
- [56. Android数字签名要点](#56-android数字签名要点)
- [57. 使用相同数字签名的原因](#57-使用相同数字签名的原因)
- [58. Theme和Sytle](#58-theme和sytle)
    - [58.1. Style](#581-style)
    - [58.2. Theme](#582-theme)
- [59. Toast的时长设置](#59-toast的时长设置)
- [60. 触发ANR的情况](#60-触发anr的情况)
- [61. ServiceConnection的`onServiceConnected()`触发条件](#61-serviceconnection的onserviceconnected触发条件)
- [62. Android虚拟设备不支持的功能](#62-android虚拟设备不支持的功能)
- [63. RemoteView的应用](#63-remoteview的应用)
- [64. Android对HashMap做了优化后推出的新的容器类是什么？](#64-android对hashmap做了优化后推出的新的容器类是什么)
    - [64.1. SparseArray](#641-sparsearray)
    - [64.2. ArrayMap](#642-arraymap)
- [65. Android安全沙盒](#65-android安全沙盒)
...
## 20. Android Service与Activity之间的通信方式？
...
## 22. 为什么要用ContentProvider？它和SQL的实现上有什么差别？
...
## 24. Android如何访问自定义ContentProvider
...
## 25. Android中Activity，Intent，Content Provider，Service各有什么区别。
...
## 28. `android:layout_gravity`和`android:gravity`的区别？
...
### 32.1. public boolean dispatchTouchEvent(MotionEvent ev)
...
### 32.2. `public boolean onInterceptTouchEvent(MotionEvent ev)`
...
### 32.3. `public boolean onTouchEvent(MotionEvent ev)`
...
## 50. Android里面为什么要设计出Bundle而不是直接用Map结构

Map里实现了Serializable接口，而在Bundle实现了Parcelable的接口。

## 51. 在Android的MVP架构中，使用了什么设计模式
...
### 54.2. 上下文菜单（Contextual Menus）

#### 54.2.1. 浮动上下文菜单（floating context menu）
...
## 61. ServiceConnection的`onServiceConnected()`触发条件

- `bindService()`方法执行成功；
- `onBind()`方法返回非空IBinder对象。

## 62. Android虚拟设备不支持的功能
...
## 64. Android对HashMap做了优化后推出的新的容器类是什么？
